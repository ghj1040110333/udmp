...
public class IssuedDistributionTaskBean {
	...
	public void distributionTask(DelegateTask task,DelegateExecution execution){
		...
		if(flag){
			...
			//指定用户无法分发,按照历史记录分发
			assigneeForHistory(engineServices, task, execution);
		}
	 ...
	 * @return
	 */
	private boolean assigneeForHistory(EngineServices engineServices,DelegateTask task,DelegateExecution execution){
		...
		List<HistoricTaskInstance> htasks = engineServices.getHistoryService()
				...
				.taskDefinitionKey(task.getTaskDefinitionKey()).orderByTaskCreateTime().desc().list();
	...
	private boolean assigneeForIssued(EngineServices engineServices,DelegateTask task,DelegateExecution execution){
		...
		Set<Expression> groups=findTaskGroupName(engineServices,task.getProcessDefinitionId(), task.getTaskDefinitionKey());
		for(Expression group:groups){
			
			List<TbSysSecurityRelUserPos> pos=tbSysSecurityRelUserPosMapperImpl.selectUserForPosition(group.getExpressionText());
			if(null!=pos){
...
//						task.setAssignee(userId);
	...
	private Set<Expression> findTaskGroupName(EngineServices engineServices,String procDefId,String taskDefId){
		...
		PvmActivity pa=(PvmActivity)activityImpl;
		TaskDefinition taskDefinition=(TaskDefinition)pa.getProperty("taskDefinition");
		Set<Expression> groups=taskDefinition.getCandidateGroupIdExpressions();
		return groups;
	}
}
