...
public class BaseProcessDefinitionDiagramLayoutResource {
  ...
  public ObjectNode getDiagramNode(String processInstanceId, String processDefinitionId,String dynamicInstanceId) {
    ...
    for (ActivityImpl activity : processDefinition.getActivities()) {
      getActivity(dynamicInstanceId,processInstanceId, activity, activityArray, sequenceFlowArray, 
  ...
  private void getActivity(String dynamicInstanceId,String processInstanceId, ActivityImpl activity, ArrayNode activityArray, 
    ...
    if ("callActivity".equals(properties.get("type"))) {
      ...
      if (callActivityBehavior != null) {
    	...
    	if(StringUtils.isNotBlank(callActivityBehavior.getProcessDefinitonKey())){
    		propertiesJSON.put("processDefinitonKey", callActivityBehavior.getProcessDefinitonKey());
    	}else if(null!=callActivityBehavior.getProcessDefinitionExpression()){
    		String processExpression=callActivityBehavior.getProcessDefinitionExpression().getExpressionText().replace("${", "").replace("}", "");
    		HistoricVariableInstance hvi=historyService.createHistoricVariableInstanceQuery().processInstanceId(dynamicInstanceId).variableName(processExpression).singleResult();
    		if(null!=hvi){
    			propertiesJSON.put("processDefinitonKey", (String)hvi.getValue());
    		}
    	}
        ...
        // last definition
        if (processInstanceArray.size() == 0 && (StringUtils.isNotEmpty(callActivityBehavior.getProcessDefinitonKey()) || null!=callActivityBehavior.getProcessDefinitionExpression())) {
          // Get last definition by key
          ProcessDefinition lastProcessDefinition = repositoryService
              .createProcessDefinitionQuery()
              .processDefinitionKey(propertiesJSON.get("processDefinitonKey").asText())
    ...
    // Nested activities (boundary events)
    for (ActivityImpl nestedActivity : activity.getActivities()) {
      getActivity(dynamicInstanceId,processInstanceId, nestedActivity, activityArray, sequenceFlowArray, 
          processInstance, highLightedFlows, subProcessInstanceMap);
    }
  }
