...
<%@page
	import="activiti_maven_project.com.git.common.vo.ActDefinitionVO"%>
<%@page
	import="activiti_maven_project.com.git.common.vo.ProcessDefinitionVO"%>
...
<body>
	...
	<td>
		...
		<div id="table-wrapper" title="查询" class="template" align="right">
			<form id="conditionForm" action="<%=basePath%>worklist/queryCandiWorkList.do" 
				method="post">
				<table>
					<tr>
						<td>客户名称：</td>
				...
				<c:forEach items="${workList.actWorkListVOs}" var="actcwl"
					...
					<tr>
						...
						<td><select id="sel">
								...
								<option value="take,${actcwl.id}">签收</option>
								<option value="pic,${actcwl.executionId},${actcwl.processDefinitionId}">流程图</option>
						</select></td>
					</tr>
				</c:forEach>
				<tr>
					<div class="um-table-page fr">
						 <a
							href="worklist/queryCandiWorkList?pageNo=1">首页</a>
						<c:choose>
							<c:when test="${page.pageNo - 1 > 0}">
								<a
									href="worklist/queryCandiWorkList?pageNo=${page.pageNo - 1}">上一页</a>
							</c:when>
							<c:when test="${page.pageNo - 1 <= 0}">
								<a href="worklist/queryCandiWorkList?pageNo=1">上一页</a>
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${page.totalPage==0}">
								<a
									href="worklist/queryCandiWorkList?pageNo=${page.pageNo}">下一页</a>
							</c:when>
							<c:when test="${page.pageNo + 1 < page.totalPage}">
								<a
									href="worklist/queryCandiWorkList?pageNo=${page.pageNo + 1}">下一页</a>
							</c:when>
							<c:when test="${page.pageNo + 1 >= page.totalPage}">
								<a
									href="worklist/queryCandiWorkList?pageNo=${page.totalPage}">下一页</a>
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${page.totalPage==0}">
								<a
									href="worklist/queryCandiWorkList?pageNo=${page.pageNo}">尾页</a>
							</c:when>
							<c:otherwise>
								<a
									href="worklist/queryCandiWorkList?pageNo=${page.totalPage}">尾页</a>
							</c:otherwise>
						</c:choose>
