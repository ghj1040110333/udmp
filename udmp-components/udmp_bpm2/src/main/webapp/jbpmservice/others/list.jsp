...
<body>
	...
	<a><img src="<%=basePath %>/images/picture.png" ></a>
     	<div class="um-images-right">
		    ...
		    <form action="usercdQuery.action?flag=1&usercd=<%=usercd%>&pageSize=<%=pageSize %>" onsubmit="return isOr()" method="post" > 
		   		 <div>	 	 			 		
			...
			<form action="otherlist.action?pageSize=<%=pageSize %>"  method="post" onsubmit="return is()">	
			<%
     ...
     <div class="um-s-box">
	  	<form action="otherlist.action?pageSize=<%=pageSize %>" method="post"> 
	...
	<div class="um-s-box">
	 	<form action="otherlist.action?usercd=<%=usercd%>&pageSize=<%=pageSize %>" method="post"> 	 	 			 		
...
<script type="text/javascript">
		...
		{		
			if(<%=_page.getPageNo()%>==1||<%=_page.getPageNo()%>==0){
				...
				window.location.href="<%=basePath%>/otherpreviouslist.action?otherpage_list=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
			}
		}
		function next()
		{	
			if(<%=_page.getPageNo()%>>=<%=_page.getTotalPage()%>){
				...
				window.location.href="<%=basePath%>/othernextlist.action?otherpage_list=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
			}
		}
		function toppage()
		{	
			if(<%=_page.getPageNo()%>==1||<%=_page.getPageNo()%>==0){
				...
				window.location.href="<%=basePath%>/othertoppagelist.action?otherpage_list=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
		}
		function endpage()
		{		
			if(<%=_page.getPageNo()%>>=<%=_page.getTotalPage()%>){
				...
				window.location.href="<%=basePath%>/otherendpagelist.action?otherpagetotle_list=<%=_page.getTotalPage()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
			}
		}
		function pageSize(){
		var pageSize= document.getElementById("pageSize").value;
		window.location.href="<%=basePath%>/otherlist.action?pageSize="+pageSize+"&usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&otherpageMap_list=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>";	
	}	
	...
	function viewphoto(){
		...
		$.dialog({
			...
			cancel: function () {
				...
				$("#btn",doc).click();
				window.location.href="otherlist.action?usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&otherpageMap_list=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";
			},
		});
	}						
</script> 
