...
<html>
	...
	<body>	
		<div id="popDiv" class="mydiv" style="display:none;">
		    ...
		    <form action="usercdQuery.action?flag=4&usercd=<%=usercd%>&pageSize=<%=pageSize %>" onsubmit="return isOr()" method="post" > 
		   		 <div>	 	 			 		
			...
			<form action="othersharelist.action?pageSize=<%=pageSize %>"  method="post" onsubmit="return is()">	
			<%
     ...
     <div class="um-s-box">
	  	<form action="othersharelist.action?pageSize=<%=pageSize %>" method="post"> 	 	 			 		
	...
	<hr>	
 	...
 	%>
		...
		<div class="um-s-box">
	 	 	<form action="<%=basePath%>/othersharelist.action?usercd=<%=usercd%>&pageSize=<%=pageSize %>" method="post"> 	 	 			 		
	...
	<div class="table-wrapper"> 
		...
		<thead>
			...
			<tr >
				...
				<td><%=t.get("id")%></td>
				<td id ="picture" value="<%=t.get("executionId")%>,<%=t.get("id")%>"><a href="javascript:void()" onclick="viewphoto()">查看详细</a></td>
			</tr>
			<%
						}
				}else{
...
<script>
	...
	function pageSize(){
		...
		window.location.href="<%=basePath%>/othersharelist.action?pageSize="+pageSize+"&usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&otherpageMap_share=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>";	
	}	
	...
	function viewphoto(){
		...
		$.dialog({
			...
			background: '#000', /* 背景色 */
			opacity: 0.5,       /* 透明度 */
			content:"url:showPic.action?processId="+id+"&taskId="+taskId+"",
			icon: 'error.gif',
			cancel: function () {
				flag=true;
				...
				$("#btn",doc).click();
				window.location.href="othersharelist.action?usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&otherpageMap_share=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";
			},
		});
	}						
</script>
