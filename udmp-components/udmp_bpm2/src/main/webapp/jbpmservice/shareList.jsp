...
<html>
	...
	%>
		<div class="um-s-box">
	 	 	<form action="<%=basePath%>/sharelist.action?usercd=<%=usercd%>&pageSize=<%=pageSize %>" method="post"> 	 	 			 		
 		...
 		<div class="um-span-right">
			<input onclick="location.href='sharelist.action?usercd=<%=usercd%>&pageSize=<%=pageSize %>'" type="button" value="列表首页" class="um-btn" />
		</div> 	
 		<hr>
		<div class="um-s-box">
	 	 	  <input id="susButt" type="button" value="获取任务" class="um-btn"/>
	 	</div>
	...
	<div class="table-wrapper"> 
		...
		<thead>
			...
			<td><input name="subBox" type="checkbox" /></td>
				...
				<td id ="x"  value="<%=t.get("id")%>,<%=usercd%>"><%=t.get("id")%></td>
				<td>
					<select id="sel">
							<option value="">--操作--</option>
							<option value="view,<%=t.get("executionId")%>,<%=t.get("id")%>">查看详细</option>
							<option value="take,<%=t.get("id")%>,<%=usercd%>">获取任务</option>
   	 				</select>
   	 			</td>  
			</tr>
			<%
...
$(function () {   //批量获取任务
	$("#susButt").click(function() { 
			var taskIdArr=''; 
		$(":checkbox:checked").parents("tr").find("#x").each(function(){											
   		 ...
   		 if(taskIdArr.length>0){
	   		 window.location.href="take.action?taskIdArr="+taskIdArr+"&pageno=<%=_page.getPageNo()%>&usercd=<%=usercd%>&pageMap=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";
   		 }else{
   		 	 alert("请先选择任务");
   		 }
...
<script>
	...
	{		
		if(<%=_page.getPageNo()%>==1||<%=_page.getPageNo()%>==0){
			...
			window.location.href="<%=basePath%>/previouslist.action?page_share=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
		}
	}
	function next()
	{		
		if(<%=_page.getPageNo()%>>=<%=_page.getTotalPage()%>){
			...
			window.location.href="<%=basePath%>/nextlist.action?page_share=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
		}
	}
	function toppage()
	{		
		if(<%=_page.getPageNo()%>==1||<%=_page.getPageNo()%>==0){
			...
			window.location.href="<%=basePath%>/toppagelist.action?page_share=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
		}
	}
	function endpage()
	{		
		if(<%=_page.getPageNo()%>>=<%=_page.getTotalPage()%>){
			...
			window.location.href="<%=basePath%>/endpagelist.action?pagetotle_share=<%=_page.getTotalPage()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
		}
	}
	function pageSize(){
		var pageSize= document.getElementById("pageSize").value;
		window.location.href="<%=basePath%>/sharelist.action?pageSize="+pageSize+"&usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap_share=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>";	
	}							
</script>
<script>
	...
	$("table.um-table select").each(function(){
				...
				if(val == "view"){
						$.dialog({
							...
							background: '#000', /* 背景色 */
							opacity: 0.5,       /* 透明度 */
							content:"url:showPic.action?processId="+id+"&taskId="+second+"",
							icon: 'error.gif',
							cancel: function () {
								flag=true;
								...
								window.location.href="sharelist.action?usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap_share=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";
							},
						});
					}
				if(val == "take"){
						location.replace("take.action?flag=1&taskIdArr="+id+"&usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>");
				}
