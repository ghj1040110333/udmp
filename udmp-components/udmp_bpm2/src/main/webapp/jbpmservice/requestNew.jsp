	...
	<div class="table-request-down">
  	<form action="submitTask.action?usercd=<%=usercd%>&pageno=<%=pageno%>&pageMap=<%=pageMap%>&bizType=<%=_bizType%>&customerName=<%=_customerName%>&customerNum=<%=_customerNum%>&pageSize=<%=pageSize %>" class="bootstrap-frm" method="post">
		<input type="hidden" name="taskId" value="<%=map.get("taskId") %>"/> 
  		<h4><%=map.get("taskName") %>意见:</h4><br/>

  		<textarea rows="3" cols="10" name="comments" ></textarea><br/>
  		<input type="submit" value="确认" class="um-btn"/>
  		<input type="button" value="撤销申请" onclick="window.location.href='cancleTask.action?taskId=<%=map.get("taskId") %>&usercd=<%=usercd%>&bizType=<%=_bizType%>&customerName=<%=_customerName%>&customerNum=<%=_customerNum%>&pageno=<%=pageno%>&pageSize=<%=pageSize %>'" class="um-btn"/>
