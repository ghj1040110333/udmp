...
<html>
  ...
  <body>
	...
	<div class="table-request-down">
  	<form action="submitTask.action?usercd=<%=usercd%>&pageno=<%=pageno%>&pageMap=<%=pageMap%>&bizType=<%=_bizType%>&customerName=<%=_customerName%>&customerNum=<%=_customerNum%>&pageSize=<%=pageSize %>" class="bootstrap-frm"  method="post">
  		...
  		<span class="d-span"><label>下一个岗位:</label><input type="text" name="nextPerson" class="um-input-request"/></span><br/>
  		路由:<input type="radio" value="forward" name="operation">前进
			<input type="radio" value="back" name="operation">后退<br>
  		<input type="submit" value="确认" class="um-btn"/>
  		<input type="button" value="撤销申请" onclick="window.location.href='cancleTask.action?taskId=<%=map.get("taskId") %>&pageMap=<%=pageMap%>&usercd=<%=usercd%>&pageno=<%=pageno%>&bizType=<%=_bizType%>&customerName=<%=_customerName%>&customerNum=<%=_customerNum%>&pageSize=<%=pageSize %>'" class="um-btn"/>
		<input type="button" value="返回" onclick="window.location.href='list.action?usercd=<%=usercd%>&pageno=<%=pageno%>&pageMap=<%=pageMap%>&bizType=<%=_bizType%>&customerName=<%=_customerName%>&customerNum=<%=_customerNum%>&pageSize=<%=pageSize %>'" class="um-btn"/>
	</form>
	</div>
<script>
