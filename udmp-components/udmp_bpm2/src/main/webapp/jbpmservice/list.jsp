...
<body>
	...
	%>
		<div class="um-s-box">
	 	 	<form action="list.action?usercd=<%=usercd%>&pageSize=<%=pageSize %>" method="post"> 	 	 			 		
	...
	<hr>	
	<div class="um-s-box">
 	 	   <input id="susButt" type="button" value="挂起" class="um-btn"/>
		   <input id="repButt" type="button" value="回池"  class="um-btn"/>
 	</div>
	<div class="um-span-right">
		<input onclick="location.href='list.action?usercd=<%=usercd%>&pageSize=<%=pageSize %>'" type="button" value="列表首页" class="um-btn" />
	</div> 
				...
				<tr>
					...
					<td>
						<select id="sel">
								...
								<option value="suspend,<%=t.get("id") %>,<%=t.get("executionId")%>">挂起</option>
								<option value="replace,<%=t.get("id") %>,<%=t.get("descr")%>">回池</option>
...
<script type="text/javascript">
		...
		{		
			if(<%=_page.getPageNo()%>==1||<%=_page.getPageNo()%>==0){
				...
				window.location.href="<%=basePath%>/previouslist.action?page_list=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
			}
		}
		function next()
		{	
			if(<%=_page.getPageNo()%>>=<%=_page.getTotalPage()%>){
				...
				window.location.href="<%=basePath%>/nextlist.action?page_list=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
			}
		}
		function toppage()
		{	
			if(<%=_page.getPageNo()%>==1||<%=_page.getPageNo()%>==0){
				...
				window.location.href="<%=basePath%>/toppagelist.action?page_list=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
		}
		function endpage()
		{		
			if(<%=_page.getPageNo()%>>=<%=_page.getTotalPage()%>){
				...
				window.location.href="<%=basePath%>/endpagelist.action?pagetotle_list=<%=_page.getTotalPage()%>&pageMap=<%=strMap%>&usercd=<%=usercd%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";	
			}
		}
		function pageSize(){
		var pageSize= document.getElementById("pageSize").value;
		window.location.href="<%=basePath%>/list.action?pageSize="+pageSize+"&usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap_list=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>";	
	}							
</script> 
...
$(function () {   //批量回池
	$("#repButt").click(function() { 
			var taskIdArr=''; 
			var executionIdArr='';  
		$(":checkbox:checked").parents("tr").children("#x").each(function(){											
					var sel=$(this).attr("value"); 
					var list=sel.split(",");
					var id=list[0];
					var executionId = list[1];
					if(executionId!=null && executionId!="" && executionId!="null")
					{
						taskIdArr += id + ',' ;	
					}
					else{
   		 ...
   		 if(executionIdArr.length>0){
   		 	alert("会签任务不能回池,请重新选择");
   		 	window.location.href='<%=basePath%>/list.action?usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap_list=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>';
   		 }
   		 else if(taskIdArr.length>0){
	   		 window.location.href="<%=basePath%>/replace.action?taskIdArr="+taskIdArr+"&pageno=<%=_page.getPageNo()%>&usercd=<%=usercd%>&pageMap=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>";
   		 }else{
   		 	 alert("请先选择任务");
   		 }
	}) ;
   		 ...
   		 if(executionIdArr.length>0){
   		 	...
   		 	window.location.href='<%=basePath%>/list.action?usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap_list=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>';
   		 		...
   		 		$.dialog({
							...
							background: '#000', /* 背景色 */
							opacity: 0.5,       /* 透明度 */
							content:"url:<%=basePath %>/jbpmservice/suspendTime.jsp?id="+taskIdArr+"",
							icon: 'error.gif',
							cancel: function () {
								flag=true;
...
<script>
	...
	$("table.um-table select").each(function(){
				...
				if(val == "suspend"){
					if(second!=null && second!="" && second!="null"){
						$.dialog({
							title:'挂起',
							lock: true,
							background: '#000', /* 背景色 */
							opacity: 0.5,       /* 透明度 */
							content:"url:<%=basePath %>/jbpmservice/suspendTime.jsp?id="+id+"",
							icon: 'error.gif',
							cancel: function () {
								flag=true;
								var win=this.iframe.contentWindow;
						...
						alert("Id为【"+id+"】的任务为会签任务，不能被挂起");
						window.location.href='<%=basePath%>/list.action?usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap_list=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>';
					}
				}								
				if(val == "replace"){
					if(second!=null && second!="" && second!="null"){
						alert("Id为【"+id+"】的任务为会签任务，不能回池");
						window.location.href='<%=basePath%>/list.action?usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap_list=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>';
					}else{
						location.replace("replace.action?taskIdArr="+id+"&usercd=<%=usercd%>&pageno=<%=_page.getPageNo()%>&pageMap=<%=strMap%>&bizType=<%=bizType%>&customerName=<%=customerName%>&customerNum=<%=customerNum%>&pageSize=<%=pageSize %>");
					}
				}
				if(val == "showPic"){
					$.dialog({
