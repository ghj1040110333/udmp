...
package cn.com.git.udmp.modules.act.web;
...
public class ActTaskController extends BaseController {
	...
	@Autowired
    private IUDMPContext udmpContext;
	...
	public String todoList(Act act, HttpServletResponse response, Model model) throws Exception {
		...
		if (udmpContext.getPrincipal().isMobileLogin()){
	...
	public String todoTask(Act act, HttpServletResponse response, Model model) throws Exception {
		List<Act> list = actTaskService.todoList(act);
		model.addAttribute("list", list);
		if (udmpContext.getPrincipal().isMobileLogin()){
			return renderString(response, list);
		}
		return "modules/act/actTaskTodoTask";
	}
	...
	public String form(Act act, HttpServletRequest request, Model model){
		// 获取流程XML上的表单KEY
		String formKey = actTaskService.getFormKey(act.getProcDefId(), act.getTaskDefKey(),act.getTaskId());
	 ...
	 * @param procDefId 流程定义ID
	 */
	@RequestMapping(value = "formAppr")
	public String formAppr(Act act, HttpServletRequest request, Model model){
		// 获取流程XML上的表单KEY
		String formKey = actTaskService.getFormKey(act.getProcDefId(), act.getTaskDefKey(),act.getTaskId());
		...
		return "redirect:" + ActUtils.getFormUrlForAppr(formKey, act);
	 ...
	 * 		vars.keys=flag,pass
	 * 		vars.values=1,true
	 * 		vars.types=S,B  @see cn.com.git.udmp.modules.act.utils.PropertyType
	 */
	@RequestMapping(value = "complete")
	@ResponseBody
	public String complete(Act act) {
		System.out.println("taskid="+act.getTaskId()+"procInsid="+act.getProcInsId());
		actTaskService.complete(act.getTaskId(), act.getProcInsId(), "", null);
		return "true";//adminPath + "/act/task";
	}
