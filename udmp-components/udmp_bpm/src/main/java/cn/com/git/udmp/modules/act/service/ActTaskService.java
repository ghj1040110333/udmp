...
import cn.com.git.udmp.modules.act.dao.ActDao;
import cn.com.git.udmp.modules.act.entity.Act;
import cn.com.git.udmp.modules.act.utils.ActUtils;
import cn.com.git.udmp.modules.act.utils.ProcessDefCache;
...
public class ActTaskService extends BaseService implements InitializingBean{
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
			...
			e.setBusinessId(pi.getBusinessKey());
			result.add(e);
		}
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	 ...
	 * @return
	 */
	public String getFormKey(String procDefId, String taskDefKey,String taskId){
		String formKey = "";
		if (StringUtils.isNotBlank(procDefId)){
			if (StringUtils.isNotBlank(taskDefKey)){
				try{
					formKey = formService.getTaskFormKey(procDefId, taskDefKey);
					//这个动作是为了可以进行表单标识中可以是用EL表达式
					Map<String, Object> data = taskService.getVariables(taskId);
					formKey = (String) ElUtils.getInstance(data).getValue(formKey);
				}catch (Exception e) {
					formKey = "";
				}
			}
	...
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars) {
		...
		if(StringUtils.isNotBlank(businessTable) ){
			bid =  businessTable+":"+businessId;
		}else{
			bid = businessId;
		}
		// 启动流程
		ProcessInstance procIns = runtimeService.startProcessInstanceByKey(procDefKey,bid, vars);
