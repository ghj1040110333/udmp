...
import cn.com.git.udmp.web.context.UdmpContext;
...
public class ActProcessServiceImpl implements ProcessService{
	...
	public String startProcessor(String procDefKey, String businessId, 
			Map<String, Object> vars) {
		String userId = udmpContext.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId())
	...
	public void backProcess(String taskId, String activityId, Map<String, Object> variables) {
		if (StringUtils.isBlank(activityId)) {  
            throw new ServiceException("驳回目标节点ID为空！");  
        }  
        // 查找所有并行任务节点，同时驳回  
	...
	public void backProcess(String taskId, Map<String, Object> variables) {
		...
		ActivityImpl lastActivity = rtnList.get(0);
		backProcess(taskId, lastActivity.getId(), variables);
	...
	public void callBackProcess(String processId, String taskDefId) {
		if (StringUtils.isBlank(taskDefId)) {  
            throw new ServiceException("目标节点ID为空！");  
        }
		 
        // 查找所有并行任务节点，同时取回  
        List<Task> taskList = taskService.createTaskQuery().processInstanceId(processId).list();  
        for (Task task : taskList) {  
            commitProcess(task.getId(), null, taskDefId);
        } 
    ...
    private ProcessDefinitionEntity findProcessDefinitionEntityByTaskId(  
        ...
        if (processDefinition == null) {  
            throw new ServiceException("流程定义未找到!");  
        }  
    ...
    private TaskEntity findTaskById(String taskId) {  
        TaskEntity task = (TaskEntity) taskService.createTaskQuery().taskId(  
                taskId).singleResult();  
        if (task == null) {  
            throw new ServiceException("任务实例未找到!");  
        }  
        return task;  
    }
