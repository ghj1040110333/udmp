		...
		<div>
			...
			<input id="endDate" name="endDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate" style="width:163px;"
				value="<fmt:formatDate value="${act.endDate}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
			&nbsp;<input id="btnSubmit" class="btn i-btn-ok" type="submit" value="查询"/>
		</div>
	</form:form>
			...
			<c:forEach items="${list}" var="act">
				...
				<tr>
					...
					<td>
						...
						<c:if test="${not empty task.assignee}">
							<a href="${ctx}/act/task/formAppr?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}&businessId=${act.businessId}">任务办理</a>
						</c:if>
						...
						<a target="_blank" href="${pageContext.request.contextPath}/act/rest/diagram-viewer?processDefinitionId=${task.processDefinitionId}&processInstanceId=${task.processInstanceId}">跟踪</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
