 ...
 *  2014/07/25 修正刷新后不关闭 dialog对话框   HuoYanpeng 
 *  		   修正li标签不可用时，也可以关闭dialog的bug
 *  		   修正点击菜单时 dialog不关闭bug
 */
var navTab = {
	...
	_contextmenu:function($obj){ // navTab右键菜单
		...
		$obj.contextMenu('navTabCM', {
			bindings:{
				reload:function(t,m){
					//找到reload的li 修正li不可用时也会关闭dialog的bug
					var $reload = m.find("[rel='reload']");
					//修正刷新后不关闭 dialog对话框   HuoYanpeng 2014/07/25 Start
					if(!($.pdialog.getCurrent() == null || $.pdialog.getCurrent() == "null") && $reload.attr("class").indexOf("disabled") == -1){
						$.pdialog.closeCurrent();
					}
					//修正刷新后不关闭 dialog对话框   HuoYanpeng 2014/07/25 End
					$this._reload(t, true);
				},
				closeCurrent:function(t,m){
					//修正li不可用时也会关闭dialog的bug
					var $mCur = m.find("[rel='closeCurrent']");
					//修正刷新后不关闭 dialog对话框   HuoYanpeng 2014/07/25 Start
					if(!($.pdialog.getCurrent() == null || $.pdialog.getCurrent() == "null") && $mCur.attr("class").indexOf("disabled") == -1){
						$.pdialog.closeCurrent();
					}
					//修正刷新后不关闭 dialog对话框   HuoYanpeng 2014/07/25 End
				...
				closeOther:function(t,m){
					...
					//修正刷新后不关闭 dialog对话框   HuoYanpeng 2014/07/25 Start
					if(!($.pdialog.getCurrent() == null || $.pdialog.getCurrent() == "null")  && $mOther.attr("class").indexOf("disabled") == -1){
						$.pdialog.closeCurrent();
					}
					//修正刷新后不关闭 dialog对话框   HuoYanpeng 2014/07/25 End
				...
				closeAll:function(t,m){
					...
					//修正刷新后不关闭 dialog对话框   HuoYanpeng 2014/07/25 Start
					if(!($.pdialog.getCurrent() == null || $.pdialog.getCurrent() == "null")  && $mAll.attr("class").indexOf("disabled") == -1){
						$.pdialog.closeCurrent();
					}
					//修正刷新后不关闭 dialog对话框   HuoYanpeng 2014/07/25 End
					$this.closeAllTab();
	...
	_closeTab: function(index, openTabid){
		//删除iframe 解决窗口关闭input不能获取焦点问题 HuoYanpeng
		//$("iframe",navTab.getCurrentPanel()).remove();
	...
	_reload: function($tab, flag){
		...
		if (flag && url) {
			...
			if ($tab.hasClass("external")){
				...
				// 获取parent对象
				var serializeArray = addHiddenAttrs($pagerForm.get(0), $panel);
				//若serializeArray为null,则不允许提交
				if(serializeArray == null && tranManageMode != "ignore"){
					return false;
				}else{
					serializeArray = $pagerForm.serializeArray();
				}
				var args = $pagerForm.size()>0 ? serializeArray : {}
			...
			if (url) {
				if ($tab.hasClass("external")) {
					...
					if ($.isEmptyObject(op.data)) { //获取pagerForm参数
						...
						if(op.data.buttonCode == null && dealSwitch){
							// 交易管理 AddBY HYP
							op.data = reloadPage(op.data);
						}
						//刷新navTab页
//						var $page=$pagerForm.parents(".unitBox");
//						var tokenval=$page.find("input[name='token']").val();
//						op.data = op.data.push({'name':'token','value':tokenval});
					}
			...
			var $box = $parent.find("#" + op.navTabId);
				$box.ajaxUrl({
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		// if dialog exists,close it By HuoYanpeng
		if(!($.pdialog.getCurrent() == null || $.pdialog.getCurrent() == "null" )){
			$.pdialog.closeCurrent();
		}
