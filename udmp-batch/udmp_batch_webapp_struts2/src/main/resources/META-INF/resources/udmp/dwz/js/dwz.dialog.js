...
(function($){
	$.pdialog = {
		_op:{height:300, width:580, minH:40, minW:50, total:20, max:false, mask:false, resizable:true, drawable:true, maxable:true,minable:true,fresh:true,isClose:true},
		_current:null,
		_zIndex:42,
		getCurrent:function(){
			//添加body上的判断，若找不到div.dialog 则认为该页面没有dialog AddBY HYP
			//此处更改后可以在其他js文件中加入判断 元素所在位置是dialog还是navTab
		...
		//打开一个层
		//增么mydada参数 LiAnDong 
		open:function(url, dlgid, title, options,mydata) {
			...
			if(dialog) {
				...
				if(op.fresh || url != $(dialog).data("url")){
					...
					var jDContent = dialog.find(".dialogContent");
					jDContent.loadUrl(url,mydata, function(){
						jDContent.find("[layoutH]").layoutH(jDContent);
						$(".pageContent", dialog).width($(dialog).width()-14);
...
//				if(op.isClose){
//					$("a.close", dialog).show().click(function(event){ 
//						$.pdialog.close(dialog);
//						return false;
//					});
					
				//点击dialog上的关闭按钮，关闭当前dialog BY HYP
				$("a.close", dialog).click(function(event){ 
...
//					$.pdialog.switchDialog(dialog);
		...
		close:function(dialog) {
			...
			//$("iframe",$.pdialog.getCurrent()).remove();
		...
		// 添加 dialog删除后dialog刷新 HuoYanpeng
