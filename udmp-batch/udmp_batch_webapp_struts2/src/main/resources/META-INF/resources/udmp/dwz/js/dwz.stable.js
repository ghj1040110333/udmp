...
(function($){
	$.fn.jTable = function(options){
		return this.each(function(){
			...
			var layoutH = parseInt($(this).attr("layoutH"))+7;
			...
			// 为table添加class属性  AddBY HYP 2015年8月5日 14:08:49
			thead.wrap("<div class='gridHeader'><div class='gridThead'><table style='width:" + (tlength - 20) + "px;' class='" + $table.attr('class')+ "'></table></div></div>");
			...
			$trs.hoverClass("hover").each(function(index){
				...
				$tr.click(function(){
					...
					//***********************  将每行的单选按钮设置为选中  ************begin
					var flag = $tr.find(":input").attr("checked");
					var type = $tr.find(":input").attr("type");
					//***********************checkbox点击***********************
					if(type=='checkbox'){
						//***********************如果点击的不是checkbox，而是tr***********************
						if(!input_click){
							//***********************选中改为不选中***********************
							if(flag){
