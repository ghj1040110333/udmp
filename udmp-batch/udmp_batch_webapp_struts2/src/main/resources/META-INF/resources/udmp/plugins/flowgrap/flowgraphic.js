var g = new graphic();
 function flowgraphic()
    ...
    flowgraphic.prototype ={
	    ...
	    set_link:function(text,mode,from,to){
	    	//判断两个元素间不能存在两条
    		var ff = $("#"+from);
    		var tt = $("#"+to);
        	var fgg = new flowgraphic();
        	fgg = ff.attr("host");
				var telement =fgg.__get_elementBy_Id(to);
				//计算第一个元素的中心位置
				var points = fgg.__cal_arrow_diffpos(ff,tt,telement.type);							
				if(points!=null)
				{
					var id= fgg.draw_line_arrow(text
						,points[0]
						,[0,0],[points[1].x,points[1].y],mode);
					fgg.__setlink(id,from,to);	
				}
				telement=points=null;
	    },
	    draw_ativity:function(typeClass,cusStyle,text,fillcolor,borderColor,borderWeight)
	    {	    
	    	cusStyle.width="110px";
	    ...
	    draw_startativity:function(text)
	    {
	     ...
	     draw_startativity:function(text,left,top)
	    {
	    	var cusStyle = {};
	    	cusStyle.zIndex = this._canvas.nIndex++;
	    	cusStyle.color = "#659900";	
	    ...
	    draw_endativity:function(text,left,top)
	    {
			var cusStyle = {};
	    	cusStyle.zIndex = this._canvas.nIndex++;
	    	cusStyle.color = "#FFFFFF";	  
        ...
        draw_processactivity:function(text,left,top)
	    {
			var cusStyle = {};
			cusStyle.zIndex = this._canvas.nIndex++;
	    	cusStyle.color = "#3385D6";	 
	     ...
	     draw_judge:function(text,left,top){ //判断节点
	    	var cusStyle = {};
	    	cusStyle.zIndex = this._canvas.nIndex++;
