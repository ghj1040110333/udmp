...
idefine(function(require, exports, module) {
	...
	var vComponents = new Vue(
			{
				...
				data : {
					...
					tableUrl : $ctx + "/batch/jobRun/list",
					items : "",
					date : ""
				},
				methods : {
					addJobRun : function(id) {
						vComponents.showForm("注册任务");
					},
					updateJobRun : function(id) {
						$
								.post(
										$ctx+ "/batch/jobRun/queryById?jobRunId="+ id,
										"jobRunId:" + id,
										function(data) {
											if (data) {
												if (data.success) {
													...
													vComponents
															.showForm(
																	...
																	function() {
																		// 设置form中的值
																		window.top.msg.jobRunObj = data.data;
																	});
													// 设置form中的值
													// setTimeout(function(){
													// window.top.msg.jobRunObj =
													// data.data;
													// },500);
												} else {
													top.window.showInfo(
					...
					deleteJobRun : function(e) {
						var selected = $('#contentTable').bootstrapTable(
								'getSelections');
						window.top.showConfirm("确定要删除这个任务吗？", function() {
							$.post($ctx + "/batch/jobRun/delete?jobRunId="
									+ selected[0].jobRunId, "jobRunId:"
									+ selected[0].jobRunId, function(data) {
								if (data) {
									if (data.success) {
					...
					startBatchAsyncJobRun : function(id) {
						$
								.post(
										$ctx
												+ "/batchAsyncJobRun/startBatchAsyncJobRun_batchAsyncJobRunAction.action?batchAsyncJobRunAVO.jobRunId="
	...
	$("#contentTable").on("click", ".table-btn-edit",
			function(e, value, row, index) {
				vComponents.startBatchAsyncJobRun($(this).data("value"));
			});
	...
	$('#updateBtn').on('click', function(e) {
		...
		vComponents.updateJobRun(selected[0].jobRunId);
	});
	...
	// index) {
	// console.log(row);
	// vComponents.updateJobRun($(this).data("value"));
	// });
