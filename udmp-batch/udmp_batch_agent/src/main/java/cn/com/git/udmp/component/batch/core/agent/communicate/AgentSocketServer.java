...
/**
 * @description agent端的socket服务端
 * @author liuliang liuliang_wb@newchina.live
 * @date 2015年2月3日 下午5:36:21
 */
@Deprecated
public class AgentSocketServer implements IAccepter{
    ...
    private JobSessionContext genJsContext(Message message) {
        ...
        // String status = message.getJob().getBasicInfo().getStatus();
        // if (status.equals("START")) {
        // jsContext.setCommand(CommandEnum.START);
        // } else if (status.equals("ABORT")) {
        // jsContext.setCommand(CommandEnum.ABORT);
        // }
        // 通过报文获取需要执行的command
    ...
    public Message accept(String port) {
        logger.debug("启动服务器{}的监听进程", port);
        if (server == null) {
            server = new NettyServer(getHandler());
        }
        try {
            server.start(Integer.valueOf(port));
        } catch (Exception e) {
            logger.error("netty服务器运行异常:{}",e);
        }
        return null;
    }

    private NettyServerHandler getHandler() {
        if (handler == null) {
            handler = new NettyServerHandler() {
                ...
                public void handle(Message message) {
                    ...
                    try {
                        if (jsContext.getCommand() != CommandEnum.START
                                && jsContext.getCommand() != CommandEnum.ABORT) {
                            // TODO batch--统计任务的数量
                        ...
                        logger.debug("agent处理请求发生异常:{}", e.getMessage());
                        //TODO 返回失败信息回去
                        reportFail(message);
                    }
                }
    ...
    */
    ...
    public void stopAccepter() {
        //回收线程池
        TaskExecutorFactory.recycleExecutor();
    }
