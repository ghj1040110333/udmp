...
(function($) {
  var Tags = function(elems) {
    ...
    elems.each(function(key, elem) {
      ...
      var input = $('<input class="input-tags form-control" />').insertAfter(
        $(elem));
      self.bind(input);
    });
  };
  ...
  Tags.prototype = {
    set: function(elems, values) {
      ...
      elems.each(function(key, elem) {
        for (var key in values) {
          var tag = $('<span class="label label-info">' + values[key] +
            '</span>');
          $(elem).after(tag);
          tag.click(click);
          Tags.value(tag);
    ...
    bind: function(elem) {
      ...
      elem.keypress(function(event) {
        ...
        var tag = $('<span class="label label-info">' + text +
          '</span>');
        self.before(tag).val('');
        tag.click(click);
        Tags.value(elem);
...
$(function() {
  $('head').append($(
    '<style>.form-tags span, .input-tags {padding: .3em .6em .3em; display: inline-block;font-size: 14px;margin: 4px 4px 0 0;line-height: 26px;font-weight: normal;}.input-tags {width: auto;}</style>'
  ));
