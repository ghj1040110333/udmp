package cn.com.git.udmp.common.persistence.mybatis;
...
/**
 * 在mybatis的原来的实现中多数据源的实现是需要定义不同的package，这里改为通过@MyBatisDao的sqlSessionFactoryBeanName来进行区分
 * 
 * mybatis的MapperScannerConfigurer类不适合重写，这个的大部分是考了他的源代码
 * @author guosg
 *
 */
public class UdmpMapperScannerConfigurer implements BeanDefinitionRegistryPostProcessor, InitializingBean, ApplicationContextAware, BeanNameAware {
...
//    scanner.setAnnotationClass(this.annotationClass); //modify by guosg 在udmp中已经确定只能使用@MyBatisDao来查找dao，这个配置废弃
    ...
    scanner.addIncludeFilter(new TypeFilter() {
		...
		public boolean match(MetadataReader metadataReader, MetadataReaderFactory metadataReaderFactory)
			...
			try{
				...
				MyBatisDao mybatisDao = AnnotationUtils.getAnnotation(clazz, MyBatisDao.class);
				if(mybatisDao!=null){
					if(mybatisDao.sqlSessionFactoryBeanName().equals(sqlSessionFactoryBeanName)){
						return true;
					}
				}
			}catch(Exception ex){
