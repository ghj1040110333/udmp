...
import cn.com.git.udmp.common.persistence.entity.RoleEntity;
...
//							}
	...
	/**
	 * @title 数据范围过滤（符合业务表字段不同的时候使用，采用exists方法）
	 * @description
	 * 
	 ...
	 */
	public static void dataScopeFilter(IBaseEntity entity, String sqlMapKey, String officeWheres, String userWheres) {

		UserEntity user = entity.getCurrentUser();
		...
		for (String where : StringUtils.split(officeWheres, ",")){
			if (RoleEntity.DATA_SCOPE_COMPANY_AND_CHILD.equals(dataScopeString)){
				// 包括本公司下的部门 （type=1:公司；type=2：部门）
			...
			else if (RoleEntity.DATA_SCOPE_COMPANY.equals(dataScopeString)){
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
				sqlString.append(" WHERE type='2'");
				sqlString.append(" AND id = '" + user.getCompany().getId() + "'");
				sqlString.append(" AND " + where +")");
			}
			else if (RoleEntity.DATA_SCOPE_OFFICE_AND_CHILD.equals(dataScopeString)){
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
			...
			else if (RoleEntity.DATA_SCOPE_OFFICE.equals(dataScopeString)){
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_OFFICE");
				sqlString.append(" WHERE id = '" + user.getOffice().getId() + "'");
				sqlString.append(" AND " + where +")");
			}
			else if (RoleEntity.DATA_SCOPE_CUSTOM.equals(dataScopeString)){
		...
		for (String where : StringUtils.split(userWheres, ",")){
			if (RoleEntity.DATA_SCOPE_SELF.equals(dataScopeString)){
	    ...
	    logger.debug("BaseService.dataScopeFilter: " + sqlString.toString());
