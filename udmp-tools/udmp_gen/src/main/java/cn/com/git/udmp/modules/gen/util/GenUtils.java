...
package cn.com.git.udmp.modules.gen.util;
...
import cn.com.git.udmp.common.mapper.JaxbMapper;
...
import cn.com.git.udmp.modules.gen.entity.GenCategory;
import cn.com.git.udmp.modules.gen.entity.GenConfig;
import cn.com.git.udmp.modules.gen.entity.GenScheme;
import cn.com.git.udmp.modules.gen.entity.GenTable;
import cn.com.git.udmp.modules.gen.entity.GenTableColumn;
 ...
 * @version 2013-11-16
 */
public class GenUtils implements ILog{
    ...
    public static <T> T fileToObject(String path,String fileName, Class<T> clazz){
        try {
...
//      }
	...
	public static <T> T fileToObject(String fileName, Class<T> clazz){
		return fileToObject("/META-INF/templates/modules/gen/", fileName, clazz);
	}
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme){
		...
		//modified by liang 为修改生成器的目录结构添加一个项目前缀参数
		Object packageName = model.get("packageName");
		String projectPrefix = "";
		if(packageName!=null&&packageName.toString().startsWith("cn.com.git.")){
		    projectPrefix=packageName.toString().substring(11);
		    projectPrefix = projectPrefix.replace('.', '/');
		}
		model.put("projectPrefix", projectPrefix);
